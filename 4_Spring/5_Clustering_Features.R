...
# 
# Input:
#   - Processed dataset with magnetic and environmental variables: 'Spring_Magnetic.csv'.
# 
# Output:
#   - Updated dataset with clustering features: 'Spring_ClusteringFeatures.csv'.
# ------------------------------------------------------------------------------
...
df <- read.csv("Spring_Magnetic.csv")
...
df <- df %>%
  ...
  mutate(
    ...
    delta_angle_to_sun = abs(calculate_delta_by_id(angle_relative_to_sun, idyear)), 
...
df <- df %>%
  rowwise() %>% 
  mutate(
    delta_angle_to_sun2 = calculate_smallest_degree_difference(delta_angle_to_sun),
...
write.csv(df,"Spring_ClusteringFeatures.csv", row.names = FALSE)
