...
selected_clusters <- c("High ΔI' & Δψm", "High ΔI", "Inland", "Low ΔI' & Δψm,Tailwinds")
selected_cluster_ids <- which(cluster_labels %in% selected_clusters)
...
df_freq <- df %>%
  group_by(cluster_id) %>%
  filter(cluster_id %in% selected_cluster_ids) %>% # Filter for the selected clusters
...
ggplot(df_freq, aes(x = heading_circular_bin, y = frequency)) +
  ...
  facet_wrap(~factor(cluster_id, levels = selected_cluster_ids, labels = selected_clusters), ncol = 4) +
...
# Save the plot
ggsave("SpringDay_CircularHistogram_Selectedones.png", width = 16, height = 4.5, dpi = 300)
...
# Selected clusters for plotting
selected_clusters <- c("High ΔI' & Δψm(2)", "High ΔI", "Inland(1)", "Low ΔI' & Δψm,Tailwinds")
selected_cluster_ids <- which(cluster_labels %in% selected_clusters)
...
# Save the plot
ggsave("SpringNight_CircularHistogram_Selectedones.png", width = 16, height = 4.5, dpi = 300)
